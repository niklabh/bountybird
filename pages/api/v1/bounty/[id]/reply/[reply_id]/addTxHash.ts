...
const handler: NextApiHandler<IApiSuccessResponse | IApiErrorResponse> = async (req, res) => {
	...
	if (!hash || typeof hash !== 'string' || hash.trim().length < 1) {
	...
	if (replyData.txHash) {
		return res.status(400).json({ error: 'Reply already has a hash' });
	}

	//check if username is same as bounty username

	if (
		(bountyData.source === 'twitter' && bountyData.username !== privyUser.twitter?.username) ||
		(bountyData.source === 'lens' && bountyData.username !== userDocData.lens_handle) ||
		(bountyData.source === 'bounty bird' && bountyData.username !== userDocData.username)
	) {
	...
	return res.status(200).json({ message: 'Tx. hash saved.' });
};
