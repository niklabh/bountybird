...
import { IApiErrorResponse, IApiSuccessResponse, IUser } from '@/src/types';
...
const handler: NextApiHandler<IApiSuccessResponse | IApiErrorResponse> = async (req, res) => {
	...
	if (userData.wishlist && !userData.wishlist.includes(bountyId)) {
		return res.status(400).json({ error: 'Bounty does not exist in wishlist.' });
	}

	const newUserData: any = {
		id: userId,
		wishlist: userData.wishlist?.filter((id) => id !== bountyId)
	};
	...
	return res.status(200).json({ message: 'Bounty removed from wishlist.' });
};
